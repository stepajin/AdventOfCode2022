...
var cache: [State: (i: Int, height: Int)] = [:]
var height = 0
var i = 0
let maxI = 1000000000000

while i < maxI {
    let rockIndex = i % rocks.count
    
    // Cache
    let state = State(rockIndex: rockIndex, jetIndex: jetIndex, space: space)
    if let cachedState = cache[state] {
        let iDiff = i-cachedState.i
        let heightDiff = height-cachedState.height
        let x = (maxI - i) / iDiff
        if x > 0 {
            i += x * iDiff
            height += x * heightDiff
    ...
    space = Array(space.dropFirst(emptyPrefix).prefix(100))

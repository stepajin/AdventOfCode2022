...
let dividerPackets: [Element] = ["[[2]]", "[[6]]"].map(readPacket(_:))
var packets: [Element] = dividerPackets
while let (packet1, packet2) = readPair() {
    packets.append(contentsOf: [packet1, packet2])
}
packets.sort()

let indexes = dividerPackets.compactMap(packets.firstIndex(of:)).map { $0 + 1 }
let key = indexes.reduce(1, *)
print(key)
